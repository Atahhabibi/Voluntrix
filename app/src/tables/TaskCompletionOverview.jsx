...
const TaskCompletionOverview = ({ tasks, isLoading, isError }) => {
  ...
  const currentTasks = tasks
    .map((task) => ({
      name: task.name || "",
      time: task.time || "",
      volunteersNeeded: task.name
        ? task.volunteersNeeded !== undefined
  ...
  if (isError) {
    return <div className="text-center text-red-400">Error loading tasks!</div>;
  }
          ...
          <tbody>
            {rows.map((task, index) => (
              ...
              >
                ...
                <td className="px-4 py-2 text-center border border-gray-700 h-10">
                  {task.volunteersNeeded !== "" ? task.volunteersNeeded : ""}
                </td>
              </tr>
            ))}
          </tbody>
